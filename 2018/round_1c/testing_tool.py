...
for i in range(NUM_TEST_CASES):
    ...
    for j in range(N):
        ...
        try:
            ...
            if user_input == "":
                exit_msg = (
                    "Read an empty string for the flavor. This might happen "
                ...
                exit_msg = (
                    "Failed to read the flavor. Expected an integer ending "
                    'with one new line character. Read "{}" (quotes added to '
                    "isolate your output) instead."
                ).format(user_input)
        ...
        if q != -1:
            ...
            sold = sold + 1
    print(
        "Case {}/{}: sold {} out of a possible {} lollipops.".format(
            i + 1, NUM_TEST_CASES, sold, MAX[i]
        )
    )

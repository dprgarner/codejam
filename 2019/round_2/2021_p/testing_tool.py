...
def Won(vases):
    ...
    win_vases_idx = [i for i, vase in enumerate(vases) if len(vase) == min_cnt]
    if len(win_vases_idx) > 1:
        print(
            "failed case: vases {} all have {} tokens, so "
    ...
    for p in win_vase:
        if win_vase.count(p) > 1:
            print(
                "failed case: winning vase {} had more than "
    ...
    if npeople not in win_vase:
        print(
            "failed case: I don't have a token in the "
        ...
        for i in range(1, npeople + 1):
            ...
            if person > 0:
                ...
                if i == npeople and person != npeople:
                    return (
                        False,
                        "The player must place their own token on the last day!",
                    )
